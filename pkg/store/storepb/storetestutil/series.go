...
func CreateHeadWithSeries(t testing.TB, j int, samplesPerSeries int, series int, random *rand.Rand) (*tsdb.Head, []storepb.Series) {
	if samplesPerSeries < 1 || series < 1 {
		t.Fatal("samples and series has to be 1 or more")
	}

	fmt.Printf("Creating %d %d-sample series:\n", samplesPerSeries, series)

	h, err := tsdb.NewHead(nil, nil, nil, int64(samplesPerSeries), "", nil, tsdb.DefaultStripeSize, nil)
	testutil.Ok(t, err)

	app := h.Appender()
	for i := 0; i < series; i++ {
		ts := int64(j*series*samplesPerSeries + i)
		ref, err := app.Add(labels.FromStrings("foo", "bar", "i", fmt.Sprintf("%07d%s", ts, LabelLongSuffix)), ts, random.Float64())
		testutil.Ok(t, err)

		for is := 1; is < samplesPerSeries; is++ {
			testutil.Ok(t, app.AddFast(ref, ts+int64(is), random.Float64()))
		}
	}
	...
	for all.Next() {
		...
		resps = append(resps, storepb.Series{
			Labels: sLset,
		})
		...
		for {
			...
			resps[len(resps)-1].Chunks = append(resps[len(resps)-1].Chunks, storepb.AggrChunk{
				MinTime: c.MinTime,
				MaxTime: c.MaxTime,
	...
	for _, c := range cases {
		t.Run(c.Name, func(t testutil.TB) {
			...
			for i := 0; i < t.N(); i++ {
				...
				testutil.Equals(t, len(c.ExpectedWarnings), len(srv.Warnings))
...
func RunSeriesInterestingCases(t testutil.TB, maxSamples, maxSeries int, f func(t testutil.TB, samplesPerSeries, series int)) {
	for _, tc := range []struct {
		...
		t.Run(fmt.Sprintf("%dSeriesWith%dSamples", tc.series, tc.samplesPerSeries), func(t testutil.TB) {
			runtime.GC()
