...
func benchBucketSeries(t testutil.TB, totalSamples, totalSeries int, requestedRatios ...float64) {
	const numOfBlocks = 4
	...
	for bi := 0; bi < numOfBlocks; bi++ {
		head, bSeries := storetestutil.CreateHeadWithSeries(t, bi, samplesPerSeriesPerBlock, seriesPerBlock, random)
	...
	for _, p := range requestedRatios {
		seriesCut := int(p * float64(totalSeries))
		if seriesCut == 0 {
			seriesCut = 1
		}
		allCut := int(p * float64(totalSeries*totalSamples))
		bCases = append(bCases, &storetestutil.SeriesCase{
			Name: fmt.Sprintf("%dof%d", allCut, totalSeries*totalSamples),
...
func TestSeries_RequestAndResponseHints(t *testing.T) {
	...
	// Create TSDB blocks.
	head, seriesSet1 := storetestutil.CreateHeadWithSeries(t, 0, 1, 2, random)
	block1 := createBlockFromHead(t, bktDir, head)
	testutil.Ok(t, head.Close())
	head2, seriesSet2 := storetestutil.CreateHeadWithSeries(t, 0, 1, 2, random)
